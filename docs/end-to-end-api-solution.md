...
The docs routes expose Swagger UI and the raw OpenAPI contract for vendors. Protected service routes are for backend-to-backend integrations. They use managed signed vendor clients or hashed local service API keys and sanitized responses, not admin Firebase credentials.
