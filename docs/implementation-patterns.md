...
- Generation must fetch Alpaca spot price as soon as a ticker is parsed. If the prompt does not include a supported options strategy or the option-chain calculation cannot complete, pass a spot-price market draft into the LLM and merge only the trusted market price/context back into the generated row.
...
- PDF is the third channel: publishing creates or reuses one saved PDF artifact for the batch. If `outputArtifacts.pdf.artifactId` still points to an existing artifact, republishing must reuse it instead of rendering another PDF. Report output should use NewLeaf green/gold branding, a structured summary, per-pick metric blocks, and risk-aware educational language.
- Social output is generated and saved with the batch as a reusable social-copy artifact. Social image/card rendering can be added as a renderer-backed artifact step, but admin-web should still store artifact IDs and reuse existing outputs.
- Script generation is the fourth channel: publishing creates or reuses a `recommendation_video` content job with `sourceType=text_to_heygen` and stores a reusable markdown script artifact. Keep the generated script editable in the Review Workspace before video generation.
