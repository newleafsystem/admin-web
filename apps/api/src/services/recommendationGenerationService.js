...
function buildPrompt({
  ...
  return {
    system: [
      ...
      'Use the supplied marketDataDraft calculations, option-chain legs, gamma context, technical indicators, and market read to write the thesis, risk notes, entry, exit, IV context, and sentiment context.',
    ...
    user: JSON.stringify({
      ...
      marketDataDrafts: marketDrafts.map((item) => ({
        ...
        lifecycle: {
          ...
          technicalIndicators: item.lifecycle?.technicalIndicators,
          strategyAdvisor: item.lifecycle?.strategyAdvisor,
